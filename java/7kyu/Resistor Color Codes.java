...
public class DecodeResistorColors {
    static final String[] colorCodes = new String[] {"black", "brown", "red", "orange", "yellow", "green", "blue", "violet", "gray", "white"};
    static final Map<String, Integer> colorCodeMap = IntStream.range(0, colorCodes.length).boxed().collect(Collectors.toMap(i -> colorCodes[i], Function.identity()));
    public static String decodeResistorColors(String bands) {
        String[] temp = bands.split(" ");
        int ohms = (colorCodeMap.get(temp[0]) * 10 + colorCodeMap.get(temp[1])) * (int)Math.pow(10, colorCodeMap.get(temp[2]));
        String tolerance = temp.length < 4 ? "20%" : "gold".equals(temp[3]) ? "5%" : "10%";
        String ohmsStr = ohms < 1000 ? String.valueOf(ohms) : ohms < 1000000 ? String.valueOf(ohms / 1000d) + "k" : String.valueOf(ohms / 1000000d) + "M";
